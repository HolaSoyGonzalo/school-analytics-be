	...
	insertBatchExams: async (examRequests) => {
		try {
			return await dbConnections.transaction(async (t) => {
				try {
					const exams = await Promise.all(examRequests.map((req) => mapRequestToExam(req, t)));
...
const mapRequestToExam = async (req, transaction) => {
	try {
		...
		if (!student) {
			throw new EntityNotFoundError("Student with email " + req["STUDENT MAIL"] + " not found");
		}
		if (!teacher) {
			throw new EntityNotFoundError("Teacher with email " + req["TEACHER MAIL"] + " not found");
		}
		if (!course) {
			throw new EntityNotFoundError("Course with name " + req["COURSE"] + " not found");
		}
		return {
